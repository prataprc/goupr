...
const (
    // Following opcodes are for memcached
    ...
    // Opcodes for UPR
    UPR_OPEN             = CommandCode(0x50) // create a logical channel.
    UPR_ADD_STREAM       = CommandCode(0x51)
    UPR_CLOSE_STREAM     = CommandCode(0x52)
    UPR_FAILOVER_LOG     = CommandCode(0x54)
    UPR_STREAM_REQ       = CommandCode(0x53)
    UPR_STREAM_END       = CommandCode(0x55)
    UPR_SNAPSHOTM        = CommandCode(0x56)
    UPR_MUTATION         = CommandCode(0x57)
    UPR_DELETION         = CommandCode(0x58)
    UPR_EXPIRATION       = CommandCode(0x59)
    UPR_FLUSH            = CommandCode(0x5a)
...
func init() {
    ...
    CommandNames[UPR_ADD_STREAM] = "UPR_ADD_STREAM"
    CommandNames[UPR_CLOSE_STREAM] = "UPR_CLOSE_STREAM"
    CommandNames[UPR_FAILOVER_LOG] = "UPR_FAILOVER_LOG"
    CommandNames[UPR_STREAM_REQ] = "UPR_STREAM_REQ"
    CommandNames[UPR_STREAM_END] = "UPR_STREAM_END"
    CommandNames[UPR_SNAPSHOTM] = "UPR_SNAPSHOTM"
...
// When a producer opens a connection UprOpen(), extra's flag should be set to
// producer.
func FLAGS_SET_PRODUCER(flags uint32) {
    flags |= 0x1
}
